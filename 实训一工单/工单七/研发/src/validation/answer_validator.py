...
def _is_principle_question(question: str) -> bool:
    compact = re.sub(r"\s+", "", question or "")
    return (
        _contains_any(compact, ["\u4ec0\u4e48\u539f\u5219", "\u54ea\u4e9b\u539f\u5219", "\u539f\u5219\u662f"])
        or ("\u575a\u6301" in compact and "\u539f\u5219" in compact)
    )


def _answer_matches_principle_question(answer: str) -> bool:
    ...
    return _contains_any(compact, principle_terms)


def _is_report_table_title_answer(answer: str) -> bool:
    compact = re.sub(r"\s+", "", answer or "")
    table_title_terms = [
        "\u80a1\u4e1c\u6743\u76ca\u53d8\u52a8\u8868",
        "\u8d44\u4ea7\u8d1f\u503a\u8868",
    ...
    return _contains_any(compact, table_title_terms)


def _question_asks_for_report_table(question: str) -> bool:
    compact = re.sub(r"\s+", "", question or "")
...
def validate_answer_quality(question: str, answer: str, retrieved_chunks: list) -> dict:
    ...
    if _is_principle_question(q) and not _answer_matches_principle_question(a):
        reasons.append("principle_question_type_mismatch")
        confidence -= 0.85

    if _is_report_table_title_answer(a) and not _question_asks_for_report_table(q):
        reasons.append("report_table_title_answer_mismatch")
        confidence -= 0.9
    ...
    hard_mismatch = any(
        reason in reasons
        for reason in [
            "principle_question_type_mismatch",
            "report_table_title_answer_mismatch",
        ]
    )
    if answer_is_grounded and confidence < QUALITY_CONFIDENCE_THRESHOLD and not hard_mismatch:
