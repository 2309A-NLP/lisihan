...
class QueryProcessingMixin:
    ...
    def _keyword_boost(self, query: str, content: str) -> float:
        ...
        if "分公司" in (query or ""):
            if "证券分公司" in content:
                boost += 180.0
            if "分公司" in content and any(term in content for term in ["名称", "地址", "营业部", "分支机构"]):
                boost += 100.0
            if re.search(r"\d+\s*家\s*证券分公司|\d+\s*个\s*证券分公司", content):
                boost += 80.0
            if any(term in content for term in ["债券", "募集资金", "发行总额", "承销"]):
                boost -= 100.0
        if "信用减值损失" in (query or ""):
            if "信用减值损失" in content:
                boost += 220.0
            if "同比" in content and any(unit in content for unit in ["亿元", "万元", "%", "％"]):
                boost += 120.0
            if re.search(r"信用减值损失[^。；\n\r]{0,80}\d+(?:\.\d+)?\s*亿元", content):
                boost += 120.0
            if any(term in content for term in ["债券", "募集资金", "承销", "发行"]):
                boost -= 140.0
        if "哑铃型" in (query or "") or "资产配置策略" in (query or ""):
            if "哑铃型" in content and "资产配置" in content:
                boost += 220.0
            if "长期利率债" in content:
                boost += 120.0
            if "权益类资产" in content:
                boost += 100.0
            if any(term in content for term in ["公司简介", "集团简介", "基本情况", "注册资本"]):
                boost -= 100.0
        if "组织架构" in (query or ""):
            if "组织架构" in content and "调整" in content:
                boost += 180.0
            if "机构与交易业务委员会" in content:
                boost += 220.0
            if "研究与机构业务委员会" in content and "交易投资业务委员会" in content:
                boost += 160.0
            if any(term in content for term in ["公司简介", "基本情况", "注册资本", "债券"]):
                boost -= 80.0
        return boost
