...
class PDFQASystem:
    ...
    def get_or_create_engine(self):
        ...
        engine = RAGEngine()
        result = engine.initialize_project_knowledge_base(force_reparse=False)
    ...
    def reindex(self):
        engine = RAGEngine()
        result = engine.initialize_project_knowledge_base(force_reparse=True)
        st.session_state.init_status = result
        st.session_state.system_initialized = result.success
    ...
    def render_sidebar(self):
        with st.sidebar:
            ...
            if st.session_state.answer_language not in LANGUAGE_LABELS:
                st.session_state.answer_language = "zh"
            st.session_state.answer_language = st.selectbox(
                "回答语言",
                options=list(LANGUAGE_LABELS),
                index=list(LANGUAGE_LABELS).index(st.session_state.answer_language),
    ...
    def render_chat(self):
        ...
        st.caption("当前系统使用 BM25 + 向量检索 + RRF 融合，并通过 Redis 保存最近 20 条短期对话。")
