...
class AnsweringMixin:
    ...
    def ask(
        ...
        other_time = max(0.0, response_time - retrieval_time - generation_time)
        accuracy = self._estimate_accuracy(
            question=question,
            answer=answer,
            retrieval_mode=retrieval_mode,
        ...
        self.logger.info(
            "ask done | question=%s | question_type=%s | intent=%s | ambiguous=%s | has_context=%s | hits=%s | retrieval=%.3f | generation=%.3f | other=%.3f | elapsed=%.3f",
            question,
            question_type,
