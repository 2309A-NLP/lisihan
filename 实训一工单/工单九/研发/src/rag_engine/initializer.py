...
class InitializationMixin:
    ...
    def initialize_from_dir(self, pdf_dir: str = None) -> bool:
        ...
        if documents:
            ...
            if hasattr(self, "graph_store"):
                self.graph_store.build_from_documents(documents)
    ...
    def initialize_project_knowledge_base(self) -> InitResult:
        try:
            ...
            graph_details = ""
            if hasattr(self, "graph_store"):
                graph_stats = self.graph_store.build_from_documents(documents)
                graph_details = (
                    f" | graph_backend={graph_stats.backend}"
                    f" | graph_nodes={graph_stats.node_count}"
                    f" | graph_relations={graph_stats.relation_count}"
                )
            self._refresh_document_metadata()
            self.is_initialized = True
            self.last_init_result = InitResult(
                ...
                "indexed",
                "已解析项目 PDF，并构建混合检索索引。",
                details=f"index={Config.COLLECTION_NAME}{graph_details}",
