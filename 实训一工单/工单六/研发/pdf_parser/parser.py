...
from .mineru_backend import parse_mineru_pdf_file
...
def parse_pdf_file(
    ...
    *,
    ...
    selected_backend = (backend or Config.PDF_PARSER_BACKEND).lower()
    if selected_backend == "mineru":
        return parse_mineru_pdf_file(
            pdf_path,
            output=output or Config.MINERU_OUTPUT_DIR,
            table_to_text=table_to_text,
            table_format=table_format,
            max_text_chars=max_text_chars,
        )
    if selected_backend != "local":
        raise ValueError(f"Unsupported PDF parser backend: {selected_backend}")
...
class PDFParser:
    def __init__(self, chunk_size: int = 500, chunk_overlap: int = 50, output_dir: str = None):
        ...
        self.backend = Config.PDF_PARSER_BACKEND
    ...
    def _load_parsed_result(self, pdf_path: str) -> Dict | None:
        ...
        try:
            ...
            if self.backend == "mineru" and parsed.get("parser_backend") != "mineru":
                return None
            return parsed
        except Exception:
            return None
