...
def validate_answer_quality(question: str, answer: str, retrieved_chunks: list) -> dict:
    ...
    compact_answer = re.sub(r"\s+", "", a)
    compact_chunks = re.sub(r"\s+", "", chunk_text)
    answer_is_grounded = bool(compact_answer and compact_answer[:80] in compact_chunks)
    ...
    if type_hint == "percentage" and not re.search(r"\d+(?:\.\d+)?\s*%|百分之", a):
        ...
        if is_amount_only or not has_list_signal:
