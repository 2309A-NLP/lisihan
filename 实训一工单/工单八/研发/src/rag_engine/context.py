...
class ContextMixin:
    ...
    def _build_context(
        ...
        graph_modes = {"graph", "graph_rag", "graph_only", "hybrid_graph"}
        use_graph = (retrieval_mode or "").lower() in graph_modes
        use_vector_store = (retrieval_mode or "").lower() not in {"graph", "graph_rag", "graph_only"}
        retrieved_results = []
        if use_vector_store:
            retrieved_results = self.vector_store.search(
                search_query,
                top_k,
                mode="hybrid" if (retrieval_mode or "").lower() == "hybrid_graph" else retrieval_mode,
        ...
        if use_graph and hasattr(self, "graph_retriever"):
            graph_hits = self.graph_retriever.search(search_query, top_k=top_k, source_file=source_file)
            graph_explain = self.graph_retriever.explain(search_query)
            analysis["graph_rag"] = graph_explain
            analysis["graph_backend"] = graph_explain.get("backend", "memory")
            analysis["graph_matched_entities"] = graph_explain.get("matched_entities", [])
            analysis["graph_relation_count"] = len(graph_explain.get("relations", []))
            for doc, score, payload in graph_hits:
