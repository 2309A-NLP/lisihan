...
def get_dynamic_weights(question: str) -> dict:
    ...
    if any(keyword in question for keyword in WORK_ORDER_7_EXACT_KEYWORDS):
        return {"bm25": 0.90, "vector": 0.10}
...
class HybridRetriever(QueryProcessingMixin, SearchCacheMixin, VectorSearchMixin, FusionMixin):
    ...
    def __init__(self, collection_name: str = None):
        ...
        self._query_synonyms = {
            ...
            "分公司": ["证券分公司", "分支机构", "营业网点", "分公司名称", "分公司地址"],
            "证券分公司": ["分公司", "证券分公司", "分支机构", "营业部"],
            "信用减值损失": ["信用减值损失", "信用减值", "减值损失", "同比增长", "亿元"],
            "资产配置策略": ["哑铃型", "长期利率债", "权益类资产", "资产负债管理", "大类资产配置"],
            "哑铃型": ["资产配置策略", "长期利率债", "权益类资产", "资产负债管理", "大类资产配置"],
            "组织架构": ["组织架构调整", "组织机构", "调整", "委员会", "机构与交易业务委员会"],
            "组织架构调整": ["组织架构", "组织机构", "调整", "委员会", "机构与交易业务委员会"],
        }
