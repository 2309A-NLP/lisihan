...
from .mineru_backend import adapt_mineru_output, mineru_content_list_path, mineru_markdown_path
...
from src.utils.wsl_mineru import run_mineru_for_pdf


def _mineru_output_complete(pdf_path: str | Path, output_dir: str | Path | None = None) -> bool:
    output_root = Path(output_dir or Config.MINERU_OUTPUT_DIR)
    return mineru_markdown_path(pdf_path, output_root).exists() and mineru_content_list_path(pdf_path, output_root).exists()
...
def parse_pdf_file(
    ...
    *,
    ...
    if Config.PDF_PARSER_BACKEND == "mineru":
        output_dir = output or Config.MINERU_OUTPUT_DIR
        if not Config.MINERU_REUSE_OUTPUT or not _mineru_output_complete(pdf_path, output_dir):
            run_mineru_for_pdf(pdf_path, output_dir)
        return adapt_mineru_output(pdf_path, output_dir=output_dir, max_text_chars=max_text_chars)
...
class PDFParser:
    ...
    def parse_pdf(self, pdf_path: str) -> List[Document]:
        ...
        mineru_cache_incomplete = (
            Config.PDF_PARSER_BACKEND == "mineru"
            and parsed_result is not None
            and (
                parsed_result.get("parser_backend") != "mineru"
                or not _mineru_output_complete(pdf_path, self.output_dir)
            )
        )
        if parsed_result is None or mineru_cache_incomplete:
